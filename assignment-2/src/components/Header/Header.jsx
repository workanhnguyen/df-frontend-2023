...
const Header = () => {
  ...
  return (
    <>
      <header className={`w-full flex justify-between items-center px-10 py-20 ${mode === 'light' ? 'bg-white' : 'bg-sub-black text-white'}`}>
        <h1 className="text-2xl">Bookstore</h1>
  
        <section className="flex items-center gap-20">
          {/* Toggle mode button */}
          <section className="flex items-center gap-8 cursor-pointer">
            <div onClick={handleToggleMode} className={`relative w-50 h-25 flex items-center p-4 ${mode === 'light' ? 'bg-primary' : 'bg-black'} rounded-full`}>
              <div ref={toggleRef} className={`absolute ${mode === 'light' ? 'left-2' : 'right-2'} w-20 h-20 bg-white rounded-full`} />
            </div>
